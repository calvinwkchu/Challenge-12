...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    var samples_result = samplesArray[0];
    
    // D1-6. Create variables that hold the otu_ids, otu_labels, and sample_values.
    otu_ids = samples_result.otu_ids
    otu_labels = samples_result.otu_labels
    sample_values = samples_result.sample_values

    // D1-7. Create the yticks for the bar chart.
    // Hint: Get the the top 10 otu_ids and map them in descending order  
    //  so the otu_ids with the most bacteria are last. 
    let yconv = otu_ids.map(y => ('OTU ' + y))
    yslice = yconv.slice(0,10).reverse();
    xslice = sample_values.slice(0,10).reverse();
    hover = otu_labels.slice(0,10).reverse();
